    ...
    <section v-else id="blog-users" class="mb-5 pb-5">
      ...
      <div class="navs">
        <a
          href
          class="mr-3"
          :class="{'active-navs': !yes, 'disabled_link': !yes}"
          @click.prevent="yes=!yes"
        >Черновики {{ Object.keys(draft).length }}</a>
        <a
          href
          :class="{'active-navs': yes, 'disabled_link': yes}"
          @click.prevent="yes=!yes"
        >Публикованные {{ Object.keys(published).length }}</a>
      </div>
      <hr />
      <div
        v-if="Object.keys(draft).length===0"
        :class="{close: yes}"
      >У вас пока что нету никаких работ</div>
          ...
          <div class="d-block mx-auto">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
              ...
              <a
                class="dropdown-item"
                href
                @click.prevent="deleteBlog(d.id, d.image.id, d.image.url, 1)"
              >Удалить</a>
            </div>
          </div>
        </div>
      ...
      <div
        v-if="Object.keys(published).length===0"
        :class="{close: !yes}"
      >У вас пока что нету опубликованных работ</div>
      <div v-else v-for="p in published" :key="p.id" class="my-blog" :class="{close: !yes}">
        ...
        <div class="dropdown">
          <div class="d-block mx-auto">
            ...
            <span v-else class="red">Отказано: {{ p.comment }}</span>
            <a
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
              ...
              <a
                class="dropdown-item"
                href
                @click.prevent="deleteBlog(p.id, p.image.id, p.image.url, 2)"
              >Удалить</a>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async deleteBlog (id, imgId, imgUrl, part) {
      ...
      await this.$store
        .dispatch('deleteBlog', ids)
        .then(res => {
          this.loading = false
          if (part === 1) {
            var removeIndex = this.draft
              .map(function (item) {
                return item.id
              })
              .indexOf(id)

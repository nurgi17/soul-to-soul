<template>
  <div class="container">
      ...
      <div class="container">
        <div class="main-feelings d-flex flex-column">
          <h4 class="font-weight-bold">
            Чем мы можем
            <span class="green-link">помочь</span> ?
          </h4>
          <h4 class="font-weight-bold">Мы здесь, чтобы помочь тебе с чем угодно! Вещи как:</h4>
          <div class="feelings d-flex flex-row justify-content-around">
            <a href class="mt-4 pt-3 mr-4" @click.prevent="slidePrev">
              <b-img-lazy
                src="https://firebasestorage.googleapis.com/v0/b/soul-to-soul.appspot.com/o/Kids%2Fprev.svg?alt=media&token=0a97fb72-0994-4fd7-9365-414b614ed563"
                width="40px"
                height="40px"
                alt="Prev arrow"
              ></b-img-lazy>
            ...
            <a href class="mt-4 pt-3" @click.prevent="slideNext">
              <b-img-lazy
                src="https://firebasestorage.googleapis.com/v0/b/soul-to-soul.appspot.com/o/Kids%2Fnext.svg?alt=media&token=6a310772-8286-4918-a03e-5d68ab25da26"
                width="40px"
                height="40px"
                alt="Next arrow"
              ></b-img-lazy>
    ...
    <section id="you-are-not-alone-1">
      <p class="text-center">Вы можете рассказать нам больше о том, с чем вам нужна помощь?</p>
      <Buttons />
    </section>
      ...
      <div class="row">
        <div v-for="f in first" :key="f.id" class="col-12 col-sm-6 mb-4 pb-2">
          <a href @click.prevent="goArticles(f.id)">
            <b-img-lazy :src="f.image.url" class="w-100" height="375px" alt="Article image"></b-img-lazy>
          </a>
...
export default {
  ...
  async mounted () {
    await this.$store
      .dispatch('fetchArticleByCategory', 2)
      .then(res => {
        this.loading = false
